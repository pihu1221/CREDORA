...
export function Navbar() {
  ...
  const navLinks = [
    { name: t('home'), path: "/" },
    { name: "Practice", path: "/ai-test", highlight: true },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20 relative">
          ...
          <div className="hidden lg:flex items-center gap-6">
            <button 
              onClick={toggleTheme}
              className="p-2 rounded-full border border-white/10 bg-white/5 text-slate-400 hover:text-blue-400 transition-all"
              title="Toggle Theme"
            >
            ...
            <button 
              ...
              className={`p-2 rounded-full border transition-all flex items-center gap-2 px-3 ${isPremium ? 'bg-green-500/10 border-green-500/30 text-green-500' : 'bg-slate-500/10 border-slate-500/30 text-slate-500'}`}
              title={isPremium ? "Switch to Normal" : "Switch to Premium"}
            >
              {isPremium ? <Shield className="w-4 h-4" /> : <ShieldOff className="w-4 h-4" />}
              <span className="text-[10px] font-black uppercase tracking-widest">{isPremium ? t('premium') : t('normal')}</span>
            </button>
          ...
          >
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              ...
              <div className="mt-4 flex flex-col gap-2 p-2">
                <button 
                  onClick={togglePremium}
                  className={`w-full flex items-center justify-center gap-3 px-3 py-3 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all ${isPremium ? 'bg-green-500/10 border-green-500/30 text-green-500' : 'bg-slate-500/10 border-slate-500/30 text-slate-500'}`}
                >
                  {isPremium ? <Shield className="w-4 h-4" /> : <ShieldOff className="w-4 h-4" />}
                  {isPremium ? 'SWITCH TO NORMAL' : 'SWITCH TO PREMIUM'}
                </button>
                {isAuthenticated ? (
                  <>
                    <Link
