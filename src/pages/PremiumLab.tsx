...
export function PremiumLab() {
  ...
  const [algoFeedback, setAlgoFeedback] = useState<string | null>(null);
  ...
  const runNodeSync = async () => {
    ...
    try {
      const response = await ai.models.generateContent({
        ...
        contents: `Generate a JSON array of 4 objects representing hypothetical companies/institutions and their 'sync' match (0-100) with a specialist in the field of ${selectedField}. 
      ...
      setSyncResults(parseAIResponse(response.text));
    } catch (e) {
      setSyncResults([
  ...
  const sendExpertMessage = async (text: string) => {
    ...
    try {
      const response = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: [
          { text: `You are ${selectedExpert.name}, an expert in ${selectedExpert.expertise}. Your bio is: ${selectedExpert.bio}. 
  ...
  const generateAlgoTest = async () => {
    ...
    try {
      const response = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: `Generate a diagnostic ${activeItem.title} test for specialization in ${selectedField} on the topic of ${algoTopic}. 
        Format as JSON with: 'title', 'problem', 'example', 'constraints'. 
        Return ONLY the JSON object. LANG: ${language}`
      });
      setAlgoQuestion(parseAIResponse(response.text));
    } catch (e) {
      setAlgoQuestion({
  ...
  const submitAlgoResponse = async () => {
    if (!algoUserResponse) return;
    setAlgoLoading(true);
    try {
      const response = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: `Evaluate this solution for the problem: ${algoQuestion?.problem}. 
        User answer: ${algoUserResponse}. 
        Provide constructive technical feedback and a score out of 100. 
        LANG: ${language}`
      });
      setAlgoFeedback(response.text);
    } catch (e) {
      setAlgoFeedback("Analysis failed. Neural link timeout.");
    }
    setAlgoLoading(false);
  };
  ...
  const runDNAMap = async () => {
    ...
    try {
      const response = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: `Generate a detailed futuristic 'Neural DNA Report' for a professional in the field of ${selectedField}. 
  ...
  const sendInterviewMessage = async (text: string) => {
    ...
    try {
      const response = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: [
          { text: `You are an AI Interviewer for a ${interviewRole} position in the ${interviewIndustry} industry. Keep it technical and slightly intimidating. Ask one deep question based on the user's input: ${text}. If the user provides an answer, provide brief feedback before asking the next question. IMPORTANT: You MUST respond in the following language: ${language}` }
        ]
      });
      setChatMessages([...newMessages, { role: 'ai', text: response.text }]);
    } catch (e) {
                              ...
                              <div className="relative group flex items-center gap-4">
                                 <div className="relative flex-1">
                                    <input 
                                       ...
                                       onKeyDown={(e) => {
                                          if (e.key === 'Enter') {
                                             sendExpertMessage(algoUserResponse);
                                             setAlgoUserResponse('');
                                          }
                                       }}
                                    />
                          ...
                          {algoLoading && !algoQuestion ? (
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
                              ...
                              <div className="lg:col-span-3 space-y-8">
                                <div className="p-8 rounded-[2.5rem] bg-black/40 border border-white/5">
                                  <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-6 text-center">Implementation Terminal</h4>
                                  <textarea 
                                    value={algoUserResponse}
                                    onChange={(e) => setAlgoUserResponse(e.target.value)}
                                    placeholder="// Transmit your logic here..."
                                    className="w-full h-[300px] bg-black/60 border border-white/10 rounded-2xl p-8 text-white font-mono text-sm outline-none focus:border-blue-500/50 transition-all placeholder:text-slate-800"
                                  />
                                  <button 
                                    onClick={submitAlgoResponse}
                                    disabled={algoLoading || !algoUserResponse}
                                    className="w-full mt-6 py-5 rounded-full bg-blue-600 text-white font-black uppercase tracking-widest text-[10px] hover:bg-blue-500 transition-all flex items-center justify-center gap-3 shadow-xl shadow-blue-600/20 disabled:opacity-50"
                                  >
                                ...
                                <AnimatePresence>
                                  {algoFeedback && (
                                    <motion.div 
                                      initial={{ opacity: 0, y: 20 }}
                                      animate={{ opacity: 1, y: 0 }}
                                      className="p-8 rounded-[2.5rem] bg-blue-600/10 border border-blue-500/20"
                                    >
                                      <h5 className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-4">Neural Evaluation Verdict</h5>
                                      <p className="text-white text-sm font-bold italic leading-relaxed">{algoFeedback}</p>
                                      <button 
                                        onClick={() => { setAlgoFeedback(null); setAlgoUserResponse(''); generateAlgoTest(); }}
                                        className="mt-6 text-blue-500 text-[9px] font-black uppercase tracking-widest border-b border-blue-500/20"
                                      >
                                        Next Protocol
                                      </button>
                                    </motion.div>
                      ...
                      <div className="p-10 rounded-[3rem] bg-black/40 border border-white/5 relative overflow-hidden group">
                        ...
                        <div className="space-y-4">
                          {['7 Operations', '6 Operations', '8 Operations', 'Impossible Protocol'].map((opt, i) => (
                             <button 
                               key={i} 
                               onClick={() => alert(`Analyzing choice ${opt}. Neural logic check in progress.`)}
                        ...
                        <div className="mt-12 flex justify-between items-center">
                           ...
                           <button className="text-blue-500 font-black text-[10px] uppercase tracking-widest border-b border-blue-500/20 hover:border-blue-500 transition-all">Submit Solution</button>
                        </div>
                          ...
                          <div className="p-8 bg-blue-600/5 border-t border-white/10">
                             <div className="relative group">
                                <input 
                                   ...
                                   onKeyDown={(e) => {
                                      if (e.key === 'Enter') {
                                         sendInterviewMessage(algoUserResponse);
                                         setAlgoUserResponse('');
                                      }
                                   }}
                                />
                                <button className="absolute right-4 top-1/2 -translate-y-1/2 p-3 rounded-xl bg-blue-600 text-white hover:bg-blue-500 transition-all shadow-lg shadow-blue-600/20">
                                   <Send className="w-6 h-6" />
                                </button>
                             </div>
