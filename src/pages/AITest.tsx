...
import { 
  ...
  ChevronDown, Check, MousePointer2, Plus
} from 'lucide-react';
...
export function AITest() {
  ...
  const questionLimit = isPremium ? 30 : 6;

  const startAssessment = async () => {
    setStatus('loading');
    try {
      const generatedQuestions = await generateTest(topic, level, questionLimit);
  ...
  const handleAnswer = (answer: string) => {
    ...
    if (currentIndex < questionLimit - 1 && currentIndex < questions.length - 1) {
  ...
  const calculateScore = () => {
    const correctItems = questions.slice(0, questionLimit).filter((q, i) => userAnswers[i] === q.correctAnswer);
    return Math.round((correctItems.length / questionLimit) * 100);
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                 <div className="md:col-span-3 p-10 rounded-[3.5rem] bg-slate-900/40 border border-white/10 backdrop-blur-2xl relative overflow-hidden shadow-2xl">
                    <div className="flex justify-between items-center mb-10">
                       <div className="flex items-center gap-4">
                          ...
                          <div>
                             <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] block mb-1">Neural Thread</span>
                             <div className="flex items-center gap-3">
                                <span className="text-xl font-black text-white italic tracking-tighter">Node {currentIndex + 1}</span>
                       ...
                       <span className="text-sm font-black text-slate-500 italic uppercase tracking-widest">
                          {Math.round(((currentIndex + 1) / questionLimit) * 100)}%
                       </span>
                    </div>
                    ...
                    <div className="relative">
                       <div className="h-2 bg-black/40 rounded-full overflow-hidden border border-white/5">
                          <motion.div 
                            initial={{ width: 0 }}
                            animate={{ width: `${((currentIndex + 1) / questionLimit) * 100}%` }}
                       ...
                       <div className="absolute top-1/2 -translate-y-1/2 left-0 right-0 flex justify-between px-1 pointer-events-none">
                          {[...Array(5)].map((_, i) => (
                             <div key={i} className={`w-1.5 h-1.5 rounded-full border transition-colors ${((currentIndex + 1) / questionLimit) * 100 > i * 25 ? 'bg-blue-400 border-blue-400' : 'bg-black border-white/20'}`} />
                          ))}
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="p-10 rounded-[3.5rem] bg-black/40 border border-white/10 flex flex-col items-center justify-center backdrop-blur-xl relative group overflow-hidden">
                    ...
                    <div className="mt-2 flex items-center gap-2">
                       ...
                       <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest">Auto-saving...</span>
                    </div>
                 </div>
              </div>
                    ...
                    {questions[currentIndex].options.map((opt, i) => {
                      ...
                      return (
                        <button 
                          ...
                          whileTap={{ scale: 0.95 }}
                          className={`group relative p-10 rounded-[2.5rem] border transition-all text-left flex items-start gap-6 overflow-hidden
                            ${isSelected 
                              ? 'bg-blue-600 border-blue-500 text-white shadow-[0_20px_50px_rgba(37,99,235,0.4)] ring-4 ring-blue-600/20 translate-y-[-4px]' 
                              : 'bg-black/40 border-white/5 text-slate-500 hover:border-white/20 hover:text-white hover:bg-slate-900/60'
                            }`}
                        >
                          <div className={`mt-1 flex-shrink-0 w-8 h-8 rounded-xl border flex items-center justify-center transition-all ${isSelected ? 'bg-white text-blue-600 border-white' : 'bg-white/5 border-white/10 text-slate-700 group-hover:border-blue-500/50 group-hover:text-blue-500'}`}>
                             {isSelected ? <Check className="w-4 h-4 text-blue-600" /> : <span className="text-[10px] font-black">{String.fromCharCode(65 + i)}</span>}
                          </div>
                          
                          <div className="flex-grow">
                             <div className={`text-[9px] uppercase tracking-widest font-black mb-3 transition-colors ${isSelected ? 'text-white/60' : 'text-slate-700 group-hover:text-blue-500'}`}>
                                Node Option {i + 1}
                             </div>
              ...
              <div className="relative z-10">
                ...
                {!isPremium && (
                  <div className="mb-12 p-6 bg-indigo-500/10 border border-indigo-500/20 rounded-[2rem] text-left relative overflow-hidden">
                    ...
                    <p className="text-slate-400 text-sm mb-6 max-w-md">You only completed 6 of 30 questions. Upgrade to Premium to access the full 30-question diagnostic and receive a formal certification.</p>
                    <Link 
                      to="/"
                ...
                <div className="flex gap-4 justify-center">
                   <button 
                     ...
                     className={`px-10 py-4 rounded-full font-black text-xs uppercase tracking-widest transition-all ${
                        theme === 'light' ? 'bg-slate-900 text-white hover:bg-slate-800' : 'bg-white text-black hover:bg-slate-200'
                     }`}
                   >
                     Test Again
                   </button>
                   <button 
                     onClick={() => window.history.back()}
                     className={`px-10 py-4 rounded-full border font-black text-xs uppercase tracking-widest transition-all ${
                        theme === 'light' ? 'bg-white border-slate-200 text-slate-800 hover:bg-slate-50 shadow-sm' : 'bg-white/5 border-white/10 text-white hover:bg-white/10'
                     }`}
                   >
                     Back to Portal
                   </button>
