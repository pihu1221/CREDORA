...
import { 
  ...
  MapPin, Calendar, Clock, CheckCircle2, ChevronRight, LayoutDashboard,
  Settings, LogOut, Bell, Users, Zap, Globe, Sparkles, Star, Trophy, Loader2, Rocket, BrainCircuit,
  Mic, MicOff, Lock, Camera, CameraOff, AlertTriangle, Info, BarChart3, Youtube, ChevronLeft,
  Handshake, Gavel, Radio, ArrowUpRight, ShieldCheck, Mail, MapPin as MapPinIcon, BookOpen as BookIcon, CheckCircle as CheckIcon
} from "lucide-react";
...
import { generateSubtopicQuestions, parseAIResponse } from "../services/geminiService";
...
export function StudentDashboard() {
  ...
  const diagnosticScore = parseInt(localStorage.getItem('student_diagnostic_score') || '0');
  ...
  const [certificates, setCertificates] = useState<any[]>(() => {
    try {
      const saved = localStorage.getItem('credo_certificates');
      const parsed = saved ? JSON.parse(saved) : [];
      return Array.isArray(parsed) ? Array.from(new Map(parsed.map((item: any) => [item.id, item])).values()) : [];
    } catch (e) {
      return [];
    }
  });
  ...
  useEffect(() => {
      ...
      if (!micEnabled || !camEnabled) {
        setProctoringWarnings(prev => {
          ...
          if (next >= 3) {
            ...
            alert("Kicked from contest due to lack of proctoring verification.");
    ...
    }, 10000); // Trigger warning every 10s if sensors off
  ...
  const submitContest = async () => {
    ...
    try {
       ...
       setDeepAnalysis(parseAIResponse(response.text));
    } catch(e) {
       setDeepAnalysis({
  ...
  const [dailyWorkHistory, setDailyWorkHistory] = useState([
    { day: 'Mon', progress: 45, completed: 2 },
    { day: 'Tue', progress: 52, completed: 3 },
    { day: 'Wed', progress: 48, completed: 1 },
    { day: 'Thu', progress: 65, completed: 4 },
    { day: 'Fri', progress: 80, completed: 5 },
    { day: 'Sat', progress: 75, completed: 2 },
    { day: 'Sun', progress: 92, completed: 6 },
  ]);
  ...
  const [hasCompletedDiagnostic, setHasCompletedDiagnostic] = useState(() => {
    return localStorage.getItem('credo_diagnostic_completed') === 'true';
  });
  ...
  const studentProfile = {
    name: user?.displayName || "Elite Agent",
    email: user?.email || "agent@creadora.io",
    role: selectedField,
    location: "Global Distributed Node",
    github: "github.com/creadora-agent",
    linkedin: "linkedin.com/in/creadora-master",
  ...
  const [dailyTasks, setDailyTasks] = useState<any[]>(() => {
    try {
      const saved = localStorage.getItem('credo_daily_tasks');
      const parsed = saved ? JSON.parse(saved) : [];
  ...
  const [stickers, setStickers] = useState<any[]>(() => {
    try {
      const saved = localStorage.getItem('credo_stickers');
      const parsed = saved ? JSON.parse(saved) : [
        { id: 'welcome', name: 'Early Adopter', icon: '🚀', date: '2024-01-01' }
      ];
      return Array.isArray(parsed) ? Array.from(new Map(parsed.map((item: any) => [item.id, item])).values()) : [];
    } catch (e) {
      return [];
  ...
  const markAllRead = () => {
    ...
    localStorage.setItem('credo_notifications', JSON.stringify(updated));
  };
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognition.onresult = (event: any) => {
        ...
        const activeInput = document.querySelector('textarea[placeholder*="Neural Transmission"]') as HTMLTextAreaElement;
        if (activeInput) {
  ...
  const sendTaskMessage = async (text: string) => {
    if (!text) return;
    const newChat = [...taskChat, { role: 'user', text }];
    setTaskChat(newChat);
    setIsTalking(true);
    ...
    try {
      const response = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: `You are the Mission Control AI. Evaluate the student's progress for this mission: ${selectedTask.title} (${selectedTask.target}). 
      ...
      setTaskChat([...newChat, { role: 'ai', text: aiResponse }]);

      if (aiResponse.includes('[STATUS: COMPLETED]')) {
        completeTask(selectedTask.id);
      }
    } catch (e) {
      setTaskChat([...newChat, { role: 'ai', text: "Signal loss detected. Retransmitting..." }]);
    }
    setIsTalking(false);
  };
  ...
  const generateShowreel = async () => {
    ...
    try {
      // Re-initialize AI to use latest key
      ...
      const explanationResponse = await currentAi.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: `Act as a world-class AI expert. 
        Topic: ${topic}. 
        Tasks: 
      ...
      const aiData = JSON.parse(explanationResponse.text);
  ...
  useEffect(() => {
    localStorage.setItem('credo_daily_tasks', JSON.stringify(dailyTasks));
  }, [dailyTasks]);

  useEffect(() => {
    localStorage.setItem('credo_stickers', JSON.stringify(stickers));
  }, [stickers]);
  ...
  const generateDailyTasks = async (resultsFromDiagnostic?: any) => {
    ...
    try {
      ...
      const tasks = JSON.parse(cleanJson).map((t: any) => ({ ...t, completed: false }));
      setDailyTasks(tasks);
    } catch (e) {
      setDailyTasks([
        { id: 1, title: "Neural Sync Practice", target: "5 Exercises", type: "practice", completed: false },
        { id: 2, title: "Architecture Refactoring", target: "1 Prototype", type: "practice", completed: false },
        { id: 3, title: "System Design Patterns", target: "2 Lectures", type: "lecture", completed: false },
      ]);
    }
  ...
  const addSticker = (id: string, name: string, icon: string) => {
    setStickers(prev => {
      if (prev.find(s => s.id === id)) return prev;
      const updated = [...prev, { id, name, icon, date: new Date().toISOString().split('T')[0] }];
      localStorage.setItem('credo_stickers', JSON.stringify(updated));
      return updated;
    });
  };
     ...
     setCourses(prev => {
        ...
        if (completedCourse) {
           ...
           setCertificates(prevCerts => {
              if (prevCerts.find(c => c.id === newCert.id)) return prevCerts;
              const updatedCerts = [...prevCerts, newCert];
              localStorage.setItem('credo_certificates', JSON.stringify(updatedCerts));
              return updatedCerts;
           });
    ...
    <div className="min-h-screen bg-brand-bg flex flex-col lg:flex-row pb-20 lg:pb-0 overflow-x-hidden selection:bg-fuchsia-500 selection:text-white">
      ...
      <div className="lg:hidden fixed bottom-16 sm:bottom-0 left-0 right-0 z-50 bg-brand-bg/80 backdrop-blur-lg border-t border-white/5 flex items-center justify-around px-2 py-3 transition-colors duration-300">
        {[
          ...
          { id: 'showreel', icon: <Youtube className="w-5 h-5" /> },
      ...
      {/* Main Content */}
      <main className="flex-grow lg:ml-64 min-h-screen pt-32 px-4 md:px-8 pb-20 overflow-x-hidden">
        ...
        {activeTab === 'roadmap' && (
          <div className="space-y-12 pb-20">
            <div className="text-center mb-16 px-4">
              ...
              <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.3em] italic mt-4">Calculated trajectory for {selectedField} Specialization</p>
            </div>
            ...
            <div className="relative max-w-5xl mx-auto px-4">
              ...
              <div className="space-y-24 relative">
                {CAREER_PATHS[selectedField]?.topics.map((topic, idx) => (
                  <motion.div 
                    key={topic.id} 
                    ...
                    <div className="p-10 rounded-[3rem] bg-indigo-600 border border-white/10 text-white relative overflow-hidden group">
                       ...
                       <div className="relative">
                          <div className={`p-8 rounded-[2.5rem] border-2 border-dashed border-white/20 flex flex-col items-center justify-center gap-6 transition-all duration-700
                             ${dailyTasks.every(t => t.completed) ? 'bg-white text-indigo-600 border-solid border-white' : 'backdrop-blur-3xl'}`}>
              ...
              <div className="p-10 md:p-16 rounded-[4rem] bg-slate-900 border border-white/5 relative overflow-hidden">
                    ...
                    <button 
                      onClick={async () => {
                        ...
                        try {
                           ...
                           setResumeData(parseAIResponse(response.text));
                        } catch (e) {
                           setResumeData({
                              ...
                              education: "B.Tech in Computational Science, IIT Sector-Alpha"
                           });
                        }
                 ...
                 {resumeData ? (
                    <motion.div 
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="bg-white p-12 md:p-20 rounded-[3rem] shadow-2xl shadow-black text-slate-900 font-sans relative"
                    >
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
                          ...
                          <div className="space-y-12">
                             ...
                             <section>
                                ...
                                <p className="text-sm font-black uppercase italic leading-loose text-slate-800">{resumeData.education}</p>
                             </section>
                       ...
                       <RadarChart data={mergedSkillData}>
                        <PolarGrid stroke={theme === 'light' ? '#e2e8f0' : '#ffffff10'} />
                          ...
                          <Radar 
                            ...
                            strokeWidth={3}
                          />
                          {(selectedJobRole && !isMappingLoading) && (
                            <Radar 
                              name="Target" 
                              dataKey="Target" 
                              stroke="#d946ef" 
        ...
        {activeTab === 'portfolio' && (
           <div className="space-y-12">
              <div className="flex justify-between items-end">
                  ...
                  <button className="px-6 py-3 rounded-xl bg-blue-600 text-white font-black text-[10px] uppercase tracking-widest hover:bg-blue-500 transition-all shadow-lg shadow-blue-600/20">
                     Share Profile
                  </button>
              </div>
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-6 md:grid-cols-8 gap-6">
                       ...
                       {Array.from({ length: 8 - stickers.length }).map((_, i) => (
                          <div key={i} className="flex flex-col items-center gap-3 opacity-20">
                             <div className="w-20 h-20 rounded-full bg-black/40 border border-dashed border-white/20 flex items-center justify-center">
                                <Lock className="w-6 h-6 text-white" />
                             </div>
                             <div className="text-[8px] font-black text-slate-800 uppercase tracking-widest">Locked Slot</div>
                          </div>
                       ))}
                    </div>
                 </div>
              ...
              <div className="glass p-10 rounded-[4rem] border border-white/5 relative overflow-hidden">
                  ...
                  <div className="flex flex-col md:flex-row gap-12 items-center md:items-start mb-16">
                      <div className="w-40 h-40 rounded-[3rem] bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-5xl font-black text-white shadow-2xl shadow-blue-600/40 border-4 border-white/10">JD</div>
                      <div className="text-center md:text-left pt-4">
                          <h2 className="text-5xl font-black italic text-white uppercase tracking-tighter mb-3 leading-none">Jane Doe</h2>
                          <div className="flex items-center gap-3 mb-6 justify-center md:justify-start">
                             <p className="text-blue-500 font-black uppercase text-xs tracking-widest">Senior Software Architect</p>
                             <span className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-pulse" />
                             <p className="text-slate-500 font-black uppercase text-xs tracking-widest">Logic Tier 9</p>
                          </div>
                          <div className="flex gap-6 justify-center md:justify-start">
                              <span className="text-[10px] text-slate-500 flex items-center gap-2 font-black uppercase tracking-widest"><MapPin className="w-3 h-3 text-blue-500" /> San Francisco</span>
                              <span className="text-[10px] text-slate-500 flex items-center gap-2 font-black uppercase tracking-widest"><Calendar className="w-3 h-3 text-blue-500" /> 5+ Years Exp</span>
                          </div>
                      </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
                      ...
                      <div className="space-y-8">
                          <div className="flex items-center gap-4 mb-8">
                             <div className="h-px bg-white/10 flex-grow" />
                             <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500 italic whitespace-nowrap">Neural Milestones</h4>
                             <div className="h-px bg-white/10 flex-grow" />
                          </div>
                          <div className="space-y-6">
                            <div className="p-6 rounded-3xl bg-white/5 border border-white/5 hover:border-blue-500/20 transition-all group">
                                <h5 className="font-black uppercase text-xs text-white mb-2 group-hover:text-blue-500 transition-colors">Decentralized Asset Tracker</h5>
                                <p className="text-[10px] text-slate-500 leading-relaxed font-medium">Built high-performance tracking system handling 1M+ req/sec. Verified for concurrency and safety by Credora AI Neural Auditor.</p>
                            </div>
                            <div className="p-6 rounded-3xl bg-white/5 border border-white/5 hover:border-blue-500/20 transition-all group">
                                <h5 className="font-black uppercase text-xs text-white mb-2 group-hover:text-blue-500 transition-colors">AI-Driven Code Auditor</h5>
                                <p className="text-[10px] text-slate-500 leading-relaxed font-medium">Integrated Gemini models to audit large monorepos for security vulnerabilities. Validated for AI implementation excellence.</p>
                            </div>
                          </div>
                      </div>
                  </div>
               ...
               {contestResult && !isContestActive && (
                  <div className="max-w-6xl mx-auto space-y-12 pb-20">
                     <div className="p-8 md:p-16 rounded-[4rem] bg-slate-900 border border-white/5 relative overflow-hidden">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                           {contestResult.analysis.slice(0, 4).map((item, i) => (
